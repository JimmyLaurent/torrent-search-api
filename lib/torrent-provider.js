...
module.exports = class TorrentProvider {
  ...
  search(query, category, limit) {
    ...
    let url = this._getUrl(category, query, 0);

    // Url is null when category given is not found
    // in provider category
    // We return nothing and like this when muultiple
    // providers have the same category (ex: Movies, TV,..),
    // we don't add results from default categories.
    if(url === null) {
    ...
    return this._ensureLogin()
      .then(() => new Promise((resolve, reject) => {
        this._search(url, pageLimit)((error, response) => {
          if (error) {
            reject(error);
          }
          else {
  ...
  getTorrentDetails(torrent) {
    if (this.scrapeDatas.torrentDetailsSelector) {
      return new Promise((resolve, reject) => this._x(torrent.desc, this.scrapeDatas.torrentDetailsSelector)((error, response) => {
        if (error) {
          reject(error);
        }
        else {
  ...
  _login(cookie) {
    return this._request(this.scrapeDatas.baseUrl + this.scrapeDatas.loginUrl, 'POST', this._getLoginBody(), false);
  }

  _request(url, method = 'GET', body = null, ensureLogin = true) {
    var p = ensureLogin ? this._ensureLogin() : Promise.resolve();
  ...
  _downloadTorrent(torrent) {
    return this._request(torrent.link).then(r => r.body);
  }

  _getPageToFetchCount(limit) {
    if (limit && limit !== 'undefined') {
      return Math.floor(limit / this.scrapeDatas.resultsPerPageCount);
    }
    return 1;
  }
  ...
  _getUrl(category, query) {
    ...
    category = _.findKey(this.scrapeDatas.categories, (c,k) => k.toUpperCase() === category.toUpperCase());
    if (category === undefined) {
      return null;
    }
    ...
    if (this.scrapeDatas.categories[category].startsWith('url:')) {
      ...
      url += format(this.scrapeDatas.searchUrl, {
  ...
  _search(url, limit) {
    return this._x(url, this.scrapeDatas.itemsSelector, this.scrapeDatas.itemSelectors)
