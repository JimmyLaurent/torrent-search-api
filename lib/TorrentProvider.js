...
module.exports = class TorrentProvider {
  ...
  initCrawler() {
    ...
    this.xray.driver(
      makeDriver(this.enableCloudFareBypass, {
        method: "GET",
        jar: this.cookieJar,
  ...
  getMagnet(torrent) {
    ...
    return this.xray(torrent.desc, this.magnetSelector);
  }
  ...
  request(url, options = {}, body = null, ensureLogin = true) {
    ...
    const req = this.enableCloudFareBypass ? cloudscraperRequest : request;
    if (ensureLogin) {
      return this.ensureLogin().then(() => req(opts));
    }
    return req(opts);
  }
