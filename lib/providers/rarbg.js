...
class Rarbg extends TorrentProvider {
  _getScrapeDatas() {
    return {
      ...
      searchUrl:
        '/pubapi_v2.php?search_string={query}&category={cat}&mode=search&format=json_extended&sort=seeders&limit=100&token=',
      categories: {
        All:
  ...
  _ensureLogin() {
    if (!this.lastLoginTime || Date.now() - this.lastLoginTime > 840000) {
      return request(
        this.scrapeDatas.baseUrl + this.scrapeDatas.getTokenUrl
      ).then(r => {
